    ...
    <div className="flex bg-[#F8FAFC] min-h-screen text-slate-800 font-sans antialiased">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 font-medium">Commercial Governance</span>
              </div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">
                Quotation Discount Approvals
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Review and decide on discount concessions requested by your sales team.
              </p>
            </div>
          ...
          <div className="bg-purple-50/70 border border-purple-200 rounded-2xl p-4 flex items-start gap-3.5 text-xs text-purple-950">
            <ShieldCheck className="w-5 h-5 text-purple-600 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <strong className="font-bold block text-purple-900">Commercial Discount Authority Tiers:</strong>
              <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-[11px] text-purple-800">
                <span>• <strong>Sales Rep:</strong> Standard pricing up to 5%</span>
                <span>• <strong>Manager:</strong> Authorized up to 15%</span>
                <span>• <strong>CEO / Admin:</strong> Required for discounts &gt; 15%</span>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    ...
                    approvals.map((appr) => {
                      const quoteNum = appr.quotationId?.quotationNumber || appr.quotationNumber || 'QTN';
                      const customerName = appr.customerId?.name || appr.customerName || appr.quotationId?.customerSnapshot?.displayName || 'Direct Client';
                      const repName = appr.requestedById?.name || appr.requestedByName || 'Sales Rep';
                      const totalRupees = appr.totalAmountPaise ? appr.totalAmountPaise / 100 : (appr.totalAmount || (appr.quotationId?.grandTotalPaise ? appr.quotationId.grandTotalPaise / 100 : 0));
                      const discountPct = appr.requestedDiscountPercent || appr.discountPercent || 0;
                      const discountRupees = (totalRupees * discountPct) / 100;

                      const isCeoTier = discountPct > MANAGER_MAX_DISCOUNT_PERCENT;

                      return (
                        <tr key={appr._id} className="hover:bg-slate-50/60 transition-colors">
                          <td className="py-3 px-4">
                            <strong className="text-slate-900 block font-mono">{quoteNum}</strong>
                            <span className="text-[11px] text-slate-500">{customerName}</span>
                          </td>
                          ...
                          <td className="py-3 px-4">
                            <span className="font-black text-purple-700 block">
                              {discountPct}% (₹{discountRupees.toLocaleString('en-IN')})
                            </span>
                            <span className="text-[10px] text-slate-400">Discount Concession</span>
                          </td>
                          ...
                          <td className="py-3 px-4 font-mono text-slate-500 text-[11px]">
                            {new Date(appr.createdAt || Date.now()).toLocaleString()}
                          </td>
                    ...
                    <tr>
                      <td colSpan={8} className="py-12 text-center text-slate-400 text-xs">
                        No {statusFilter.toLowerCase()} discount approvals found.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-scale-up">
            ...
            <div className="text-center space-y-1">
              <h3 className="text-base font-bold text-slate-900">Confirm Discount Approval</h3>
              <p className="text-xs text-slate-500">
                You are approving a {selectedApproval.requestedDiscountPercent || selectedApproval.discountPercent || 0}% discount for quotation{' '}
                <strong className="text-slate-800">{selectedApproval.quotationId?.quotationNumber || selectedApproval.quotationNumber || 'QTN'}</strong>.
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-scale-up">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">Reject Discount Request</h3>
              <button
                onClick={() => {
                  setSelectedApproval(null);
                  setActionType(null);
            ...
            <form onSubmit={handleReject} className="space-y-3.5 text-xs">
              <div>
                <label className="text-slate-700 font-semibold block mb-1">Mandatory Rejection Reason *</label>
                <textarea
                  rows={3}
                  required
