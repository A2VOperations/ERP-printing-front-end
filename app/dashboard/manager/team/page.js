...
export default function MyTeamPage() {
  ...
  const loadTeamData = useCallback(async () => {
    try {
      setLoading(true);
      const [usersRes, leadsRes, flwRes, quotesRes, ordersRes] = await Promise.allSettled([
        api.get('/users'),
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 font-medium">Sales Representatives</span>
              </div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">
                My Team Directory
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Monitor workload distribution, active pipelines, and performance across your sales team.
              </p>
            </div>
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex items-center justify-between gap-4">
            ...
            <span className="text-xs font-bold text-slate-500">
              {filteredMembers.length} Representative{filteredMembers.length !== 1 ? 's' : ''}
            </span>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    ...
                    filteredMembers.map((member) => {
                      ...
                      const mQuotes = quotations.filter((q) => (q.createdById?._id || q.createdById) === member._id).length;
                      ...
                      return (
                        <tr key={member._id} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="py-3 px-4 text-slate-600">
                            <span className="font-semibold block capitalize">{member.roleSlug || member.role || 'Sales'}</span>
                            <span className="text-[10px] text-slate-400 font-mono">{member.areaCode || 'Delhi NCR'}</span>
                          </td>
