...
export default function TeamMemberDetailPage() {
  ...
  const loadMemberProfile = useCallback(async () => {
    ...
    try {
      ...
      const [userRes, leadsRes, flwRes, quotesRes, ordersRes] =
      ...
      if (leadsRes.status === "fulfilled" && leadsRes.value?.data) {
        ...
        setLeads(
          list.filter(
            (l) => (l.assignedToId?._id || l.assignedToId) === userId,
          ),
        );
      }
      if (flwRes.status === "fulfilled" && flwRes.value?.data) {
        ...
        setFollowups(
          list.filter(
            (f) => (f.assignedToId?._id || f.assignedToId) === userId,
          ),
        );
      }
      if (quotesRes.status === "fulfilled" && quotesRes.value?.data) {
        ...
        setQuotations(
          list.filter((q) => (q.createdById?._id || q.createdById) === userId),
        );
      }
      if (ordersRes.status === "fulfilled" && ordersRes.value?.data) {
        ...
        setOrders(
          list.filter((o) => (o.createdById?._id || o.createdById) === userId),
        );
      }
    } catch (err) {
  ...
  const overdueFlws = followups.filter(
    (f) =>
      f.scheduledAt &&
      new Date(f.scheduledAt) < new Date() &&
      f.status !== "COMPLETED",
  ).length;
  const totalOrderValuePaise = orders.reduce(
    (sum, o) =>
  ...
  const conversionRate =
    leads.length > 0
      ? ((wonLeadsCount / leads.length) * 100).toFixed(1)
      : "0.0";
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                <div className="text-right">
                  ...
                  <span className="text-lg font-black text-emerald-600">
                    ₹
                    {(totalOrderValuePaise / 100).toLocaleString("en-IN", {
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
              <div className="p-4 rounded-md bg-blue-50/70 border border-blue-100 space-y-1">
                ...
                <span className="text-[10px] text-slate-500 block">
                  {wonLeadsCount} won to date
                </span>
              </div>

              <div className="p-4 rounded-md bg-amber-50/70 border border-amber-100 space-y-1">
                <span className="text-amber-700 font-semibold block">
                  Overdue Follow-ups
                </span>
                <span className="font-black text-amber-900 text-xl">
                  {overdueFlws}
                </span>
                <span className="text-[10px] text-slate-500 block">
                  {followups.length} total touchpoints
                </span>
              </div>

              <div className="p-4 rounded-md bg-purple-50/70 border border-purple-100 space-y-1">
              ...
              <div className="p-4 rounded-md bg-emerald-50/70 border border-emerald-100 space-y-1">
                ...
                <span className="text-[10px] text-slate-500 block">
                  {wonLeadsCount} won out of {leads.length} leads
                </span>
              </div>
            </div>
          </div>
