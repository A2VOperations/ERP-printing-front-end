...
export default function LeadsDashboardPage() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      if (flwRes.status === 'fulfilled') {
        ...
        const arr = Array.isArray(raw) ? raw : (raw?.followUps || raw?.followups || raw?.data || []);
        setFollowups(arr);
      }
  ...
  const userRole = useMemo(() => {
    return (currentUser?.roleSlug || currentUser?.role || (typeof window !== 'undefined' ? localStorage.getItem('userRole') : '') || '').toLowerCase();
  }, [currentUser]);
  ...
  const computedMetrics = useMemo(() => {
    ...
    const proposalCount = leads.filter((l) => l.status === 'QUOTATION_SENT').length;
    const negotiationCount = leads.filter((l) => l.status === 'NEGOTIATION').length;
    const inCookingCount = leads.filter((l) => ['INTERESTED', 'FOLLOW_UP'].includes(l.status)).length;
    const wonCount = leads.filter((l) => l.status === 'WON').length;
    const lostCount = leads.filter((l) => ['LOST', 'NOT_INTERESTED'].includes(l.status)).length;

    const hotLeads = leads.filter((l) => l.priority === 'URGENT' || l.priority === 'HIGH');
    const highTicketLeads = leads.filter((l) => (l.expectedValue || l.estimatedBudget || 0) >= 20000);

    const totalExpectedVal = leads.reduce((sum, l) => sum + (l.expectedValue || l.estimatedBudget || 0), 0);
    const highTicketTotalVal = highTicketLeads.reduce((sum, l) => sum + (l.expectedValue || l.estimatedBudget || 0), 0);
  ...
  const overviewMetrics = useMemo(() => {
    ...
    const proposalCount = sourceLeads.filter((l) => l.status === 'QUOTATION_SENT').length;
    const negotiationCount = sourceLeads.filter((l) => l.status === 'NEGOTIATION').length;
    const inCookingCount = sourceLeads.filter((l) => ['INTERESTED', 'FOLLOW_UP'].includes(l.status)).length;
    const wonCount = sourceLeads.filter((l) => l.status === 'WON').length;
    const lostCount = sourceLeads.filter((l) => ['LOST', 'NOT_INTERESTED'].includes(l.status)).length;
  ...
  const donutGradient = useMemo(() => {
    ...
    segments.forEach((seg) => {
      if (seg.count > 0) {
        ...
        parts.push(`${seg.color} ${currentDeg.toFixed(1)}deg ${(currentDeg + deg).toFixed(1)}deg`);
        currentDeg += deg;
      }
    });
  ...
  const getLeadScore = (lead) => {
    ...
    if (lead.priority === 'URGENT' || val >= 50000) return { score: 95, label: 'Very High Score', color: 'text-rose-600', ring: 'border-rose-500' };
    if (lead.priority === 'HIGH' || val >= 25000) return { score: 90, label: 'Very High Score', color: 'text-purple-600', ring: 'border-purple-500' };
    if (val >= 15000) return { score: 70, label: 'High Score', color: 'text-amber-600', ring: 'border-amber-500' };
    if (val >= 8000) return { score: 55, label: 'Medium Score', color: 'text-amber-500', ring: 'border-amber-400' };
    return { score: 30, label: 'Low Score', color: 'text-blue-500', ring: 'border-blue-400' };
  };

  // Lead tag badge helper
  const getLeadTag = (lead) => {
    if (lead.priority === 'URGENT') return { label: 'HOT LEAD', icon: Flame, color: 'text-rose-600' };
    if ((lead.expectedValue || lead.estimatedBudget || 0) >= 30000) return { label: 'HIGH TICKET', icon: Star, color: 'text-purple-600' };
    if (['INTERESTED', 'FOLLOW_UP'].includes(lead.status)) return { label: 'IN COOKING', icon: CookingPot, color: 'text-amber-600' };
    if (lead.priority === 'HIGH') return { label: 'WARM LEAD', icon: Flame, color: 'text-amber-500' };
  ...
  const handleScheduleFollowup = async (e) => {
    ...
    try {
      ...
      setFollowupForm({ scheduledAt: '', title: 'Follow-up Call', description: '' });
      loadDashboardData();
    } catch (err) {
  ...
  const formatNextFollowup = (dateStr) => {
    ...
    if (diffDays === 0) return { label: `Today, ${timeStr}`, isOverdue: false, isToday: true };
    if (diffDays === 1) return { label: `Tomorrow, ${timeStr}`, isOverdue: false, isToday: false };
    if (diffDays === -1) return { label: `Overdue (Yesterday)`, isOverdue: true, isToday: false };
    if (diffDays < -1) return { label: `Overdue (${Math.abs(diffDays)}d ago)`, isOverdue: true, isToday: false };

    const monthDay = target.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    return { label: `${monthDay}, ${timeStr}`, isOverdue: false, isToday: false };
  };
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Leads Dashboard</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              ...
              <div className="text-2xl font-black text-slate-900">{computedMetrics.total}</div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">Hot Leads</span>
                <div className="w-6 h-6 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center">
                  <Flame className="w-3.5 h-3.5" />
                </div>
              </div>
              <div className="text-2xl font-black text-slate-900">{computedMetrics.hotCount}</div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">In Cooking</span>
                <div className="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center">
                  <CookingPot className="w-3.5 h-3.5" />
                </div>
              </div>
              <div className="text-2xl font-black text-slate-900">{computedMetrics.inCooking}</div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">High Ticket</span>
                <div className="w-6 h-6 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center">
                  <Star className="w-3.5 h-3.5" />
                </div>
              </div>
              <div className="text-2xl font-black text-slate-900">{computedMetrics.highTicketCount}</div>
              <div className="flex items-center gap-1 text-[10px] font-semibold text-purple-600">
                <span>₹{computedMetrics.highTicketTotalVal.toLocaleString('en-IN')} pipeline</span>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">Won Leads</span>
                <div className="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                </div>
              </div>
              <div className="text-2xl font-black text-slate-900">{computedMetrics.won}</div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">Total Expected Value</span>
                <div className="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
              ...
              <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-500">
                <span>
                  Avg ₹{computedMetrics.total > 0 ? Math.round(computedMetrics.totalExpectedVal / computedMetrics.total).toLocaleString('en-IN') : 0}/lead
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredLeads.length > 0 ? (
                  filteredLeads.slice(0, visibleCount).map((lead) => {
                    ...
                    const expVal = lead.expectedValue || lead.estimatedBudget || 0;

                    const nextFollowupItem = followups.find(
                      (f) =>
                        ((f.leadId?._id || f.leadId) === lead._id || (f.lead?._id || f.lead) === lead._id) &&
                        f.status === 'PENDING'
                    );
                    const followupInfo = formatNextFollowup(lead.nextFollowUp || nextFollowupItem?.scheduledAt);

                    return (
                      <div
                        key={lead._id}
                        className="bg-white rounded-2xl border border-slate-200/90 p-4 shadow-xs hover:shadow-md transition-all space-y-3.5 flex flex-col justify-between"
                      >
                        {/* Card Header Tag & Quick Action */}
                        <div>
                          <div className="flex items-center justify-between pb-1">
                            <div className="flex items-center gap-1.5">
                              <TagIcon className={`w-3.5 h-3.5 ${tagInfo.color}`} />
                              <span className={`text-[10px] font-extrabold uppercase tracking-wider ${tagInfo.color}`}>
                                {tagInfo.label}
                              </span>
                            </div>
                              ...
                              {isManagerOrAdmin && (
                                <button
                                  onClick={() => {
                                    ...
                                    setReassignTargetId(lead.assignedToId?._id || lead.assignedToId || '');
                          ...
                          <div className="pt-1">
                            <Link href={`/dashboard/leads/${lead._id}`} className="hover:text-indigo-600 transition-colors">
                              <h3 className="font-bold text-slate-900 text-sm leading-snug">
                                {lead.businessName || lead.companyName || lead.contactName || 'Direct Ingestion'}
                              </h3>
                            </Link>
                          ...
                          <div className="flex items-center justify-between pt-3 pb-2 border-b border-slate-100">
                            <div>
                              ...
                              <span className="text-[10px] text-slate-400 font-medium block">Expected Value</span>
                            </div>
                            ...
                            <div className="text-center">
                              ...
                              >
                                <span className={`text-xs font-black ${scoreInfo.color}`}>
                                  {scoreInfo.score}
                                </span>
                              </div>
                          ...
                          <div className="space-y-1.5 pt-2 text-[11px]">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-slate-800 font-semibold truncate max-w-[130px]">
                                {lead.assignedToId?.name || (typeof lead.assignedToId === 'string' ? 'Assigned' : 'Unassigned')}
                              </span>
                            </div>
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <span className="text-slate-400 font-medium shrink-0">Requirement:</span>
                            ...
                            <div className="flex items-center justify-between">
                              <span className="text-slate-400 font-medium">Last Contact:</span>
                              <span className="text-slate-700 font-medium">
                                {lead.lastContactedAt ? new Date(lead.lastContactedAt).toLocaleDateString() : 'No Contact Yet'}
                              </span>
                            </div>

                            <div className="flex items-center justify-between">
                              <span className="text-slate-400 font-medium">Next Follow-up:</span>
                              {followupInfo ? (
                                <span
                                  ...
                                  title={new Date(lead.nextFollowUp || nextFollowupItem?.scheduledAt).toLocaleString()}
                                >
                                  {followupInfo.label}
                                </span>
                              ) : (
                  ...
                  <div className="col-span-3 py-16 text-center text-slate-400 text-xs bg-white rounded-3xl border border-slate-200">
                    No leads matching current filters. Click &quot;+ Add Lead&quot; to register an inquiry.
                  </div>
                )}
              </div>
              ...
              {visibleCount < filteredLeads.length && (
                <div className="text-center pt-2">
                  ...
                  >
                    Load More Leads ({filteredLeads.length - visibleCount} remaining) <ChevronDown className="w-3.5 h-3.5" />
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-4 space-y-4">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs space-y-3.5">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  <h3 className="font-bold text-slate-900 text-xs">Hot Leads (Act Now!)</h3>
                  <button
                ...
                <div className="space-y-3">
                  {computedMetrics.hotLeadsList.length > 0 ? (
                    computedMetrics.hotLeadsList.map((hl) => {
                      ...
                      const timeDisplay = formatFriendlyTime(hl.createdAt || hl.updatedAt);

                      return (
                        <div key={hl._id} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div className="w-7 h-7 rounded-full bg-rose-50 text-rose-600 font-bold text-[10px] flex items-center justify-center shrink-0">
                              {(hl.contactName || hl.businessName || 'HL').slice(0, 2).toUpperCase()}
                            </div>
                    ...
                    <div className="text-slate-400 text-xs text-center py-4">No active hot leads.</div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs space-y-4">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  <h3 className="font-bold text-slate-900 text-xs">Lead Status Overview</h3>
                  <select
                    value={timeframe}
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  >
                    <div className="w-full h-full rounded-full bg-white flex flex-col items-center justify-center text-center shadow-xs">
                      ...
                      <span className="text-[8px] font-bold text-slate-400 uppercase tracking-tight">Total Leads</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 text-[10px] font-medium flex-1">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-slate-600">
                        <span className="w-2 h-2 rounded-full bg-blue-500" /> In Cooking
                      </span>
                      <strong className="text-slate-800">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs space-y-3.5">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  ...
                  <Link href="/dashboard/followups" className="text-[11px] font-bold text-indigo-600 hover:underline">
                    View All
                  </Link>
                </div>
                  ...
                  {todayFollowups.length > 0 ? (
                    <>
                      {todayFollowups.slice(0, 4).map((flw) => {
                        const timeStr = flw.scheduledAt || flw.dueAt || flw.date;
                        const timeDisplay = timeStr
                          ? `Today, ${new Date(timeStr).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
                          : 'Today';
                        const leadObj = flw.leadId && typeof flw.leadId === 'object' ? flw.leadId : null;
                        const leadTitle = flw.title || leadObj?.contactName || leadObj?.businessName || 'Follow-up Task';
                        const statusLabel = flw.status || 'SCHEDULED';

                        return (
                          <div key={flw._id} className="flex items-center justify-between text-xs">
                    ...
                    <div className="text-center py-5 text-slate-400">
                      ...
                      <p className="text-[11px] font-medium text-slate-500">No follow-ups due today</p>
                      <Link
                        href="/dashboard/followups"
          ...
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg p-6 space-y-4 shadow-2xl animate-scale-up max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateLead} className="space-y-3.5 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Customer Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Company / Brand Name</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Expected Value (₹)</label>
                  <input
                    type="number"
                    placeholder="25000"
                    value={newLead.expectedValue}
                    onChange={(e) => setNewLead({ ...newLead, expectedValue: e.target.value })}
                ...
                {isManagerOrAdmin ? (
                  ...
                  <div className="w-full px-3.5 py-2 rounded-xl bg-slate-100 border border-slate-200 text-slate-700 font-medium flex items-center justify-between">
                    <div className="flex items-center gap-2 truncate">
                      ...
                      <span className="truncate">{currentUser?.name || 'Current User'} (You)</span>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-scale-up">
            ...
            <form onSubmit={handleReassignLead} className="space-y-3.5 text-xs">
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                <span className="text-[10px] text-slate-400 font-semibold uppercase block">Selected Lead</span>
                <strong className="text-slate-900 text-sm block mt-0.5">
                  {selectedLead.businessName || selectedLead.contactName || 'Lead'} ({selectedLead.phone})
                </strong>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-scale-up">
            ...
            <form onSubmit={handleScheduleFollowup} className="space-y-3.5 text-xs">
              <div>
                <label className="text-slate-700 font-semibold block mb-1">Follow-up Title *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-slate-700 font-semibold block mb-1">Agenda / Description</label>
                <textarea
                  rows={2}
