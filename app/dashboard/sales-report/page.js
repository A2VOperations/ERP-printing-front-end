...
export default function SalesReportPage() {
  ...
  const loadReportData = useCallback(async () => {
    try {
      setLoading(true);
      const [quoteRes, orderRes, payRes, leadRes, leaderRes] = await Promise.allSettled([
      ...
      if (orderRes.status === 'fulfilled' && orderRes.value?.data) {
        setOrders(Array.isArray(orderRes.value.data) ? orderRes.value.data : (orderRes.value.data.items || []));
      }
      ...
      if (leadRes.status === 'fulfilled' && leadRes.value?.data) {
        setLeads(Array.isArray(leadRes.value.data) ? leadRes.value.data : (leadRes.value.data.leads || []));
      }
  ...
  const { startCurrent, endCurrent } = useMemo(() => getPeriodDates(timeframe), [timeframe]);
  ...
  const totalQuotationValueRupees = useMemo(() => {
    return (
      filteredQuotations.reduce((sum, q) => sum + (q.grandTotalPaise || 0), 0) / 100
    );
  }, [filteredQuotations]);
  ...
  const categoryBreakdown = useMemo(() => {
    ...
    orders.forEach((o) => {
      (o.items || []).forEach((it) => {
        ...
        counts[cat] = (counts[cat] || 0) + (it.itemTotalPaise ? it.itemTotalPaise / 100 : 1);
      });
    });
  ...
  const summaryBuckets = useMemo(() => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const qVal = qInChunk.reduce((s, q) => s + (q.grandTotalPaise || 0), 0) / 100;
      const oVal =
        oInChunk.reduce((s, o) => s + (o.grandTotalPaise || (o.grandTotal ? o.grandTotal * 100 : 0)), 0) / 100;
      const pVal =
        pInChunk.reduce((s, p) => s + (p.amountPaise || (p.amount ? p.amount * 100 : 0)), 0) / 100;
    ...
    return buckets;
  }, [startCurrent, endCurrent, filteredQuotations, confirmedOrders, filteredPayments]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Detailed live overview of sales performance, revenues, and collections
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-semibold text-slate-500">Total Quotation Value</span>
              ...
              <div>
                <div className="text-xl font-bold text-slate-900">
                  ₹{Math.round(totalQuotationValueRupees).toLocaleString('en-IN')}
                </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-semibold text-slate-500">Total Collection</span>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-semibold text-slate-500">Total Profit (Est.)</span>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-semibold text-slate-500">Orders Count</span>
                <div className="w-7 h-7 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center">
                  <Users className="w-3.5 h-3.5" />
                </div>
              </div>
              <div>
                <div className="text-xl font-bold text-slate-900">{ordersCount}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold text-slate-900">Sales Summary Breakdown</h3>
                  <span className="text-[10px] font-semibold text-slate-400">Live Period Groups</span>
                </div>
                ...
                <div className="overflow-x-auto min-h-[160px]">
                  {summaryBuckets.length === 0 ? (
                    <div className="py-12 text-center text-slate-400">
                      ...
                      <span className="text-xs font-medium block">No sales activity in this period</span>
                    </div>
                  ) : (
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {summaryBuckets.map((row, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-3 font-semibold text-slate-900">{row.date}</td>
                            <td className="py-3 text-slate-700">{row.quotValue}</td>
                            <td className="py-3 font-semibold text-slate-900">{row.orderValue}</td>
                            <td className="py-3 text-slate-700">{row.collection}</td>
                            <td className="py-3 text-emerald-600 font-semibold">{row.profit}</td>
                            <td className="py-3 text-right font-bold text-slate-900">{row.orders}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="border-t-2 border-slate-200 font-bold text-slate-900 text-xs">
                          ...
                          <td className="pt-3">₹{Math.round(totalQuotationValueRupees).toLocaleString('en-IN')}</td>
                          <td className="pt-3">₹{Math.round(totalOrderValueRupees).toLocaleString('en-IN')}</td>
                          <td className="pt-3">₹{Math.round(totalCollectionRupees).toLocaleString('en-IN')}</td>
                          <td className="pt-3 text-emerald-600">₹{Math.round(totalProfitRupees).toLocaleString('en-IN')}</td>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                <span>* Profit is estimated based on standard operating cost markup.</span>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-4">
                ...
                <div className="space-y-3">
                  {leaderboard.length === 0 ? (
                    <div className="py-8 text-center text-slate-400">
                      ...
                      <span className="text-xs font-medium block">No rankings recorded</span>
                    </div>
                  ) : (
                        ...
                        <div key={exec.userId || idx} className="space-y-1">
                          ...
                          <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${Math.min(100, exec.achievementPercent || 0)}%` }}
                            />
                          </div>
                        </div>
                      );
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs space-y-2">
                <h3 className="text-xs font-bold text-slate-900 mb-2">Live Insights</h3>
                ...
                <div className="p-2.5 rounded-xl bg-emerald-50/80 border border-emerald-100 flex items-start gap-2 text-xs">
                  ...
                  <p className="text-[11px] text-emerald-800 leading-tight">
                    Total revenue collected is <span className="font-bold">₹{Math.round(totalCollectionRupees).toLocaleString('en-IN')}</span> across {filteredPayments.length} recorded payments.
                  </p>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-blue-50/80 border border-blue-100 flex items-start gap-2 text-xs">
                  ...
                  <p className="text-[11px] text-blue-800 leading-tight">
                    <span className="font-bold">{filteredQuotations.length}</span> quotations valued at <span className="font-bold">₹{Math.round(totalQuotationValueRupees).toLocaleString('en-IN')}</span> active in this timeframe.
                  </p>
                </div>
              </div>
            </div>
