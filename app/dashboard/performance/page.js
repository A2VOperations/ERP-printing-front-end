...
function getPeriodDates(timeframe) {
  ...
  if (timeframe === 'Last Month') {
    ...
    const endCurrent = new Date(prevMonthYear, prevMonth + 1, 0, 23, 59, 59, 999);
    ...
    return { startCurrent, endCurrent, startPrev, endPrev, periodLabel: 'prior month' };
  }
  ...
  if (timeframe === 'Quarter to Date' || timeframe === 'This Quarter') {
    ...
    const startPrev = new Date(prevQuarterYear, prevQuarterIndex * 3, 1, 0, 0, 0, 0);
    const endPrev = new Date(prevQuarterYear, prevQuarterIndex * 3 + 3, 0, 23, 59, 59, 999);

    return { startCurrent, endCurrent, startPrev, endPrev, periodLabel: 'last quarter' };
  }
  ...
  if (timeframe === 'Year to Date' || timeframe === 'This Year') {
    ...
    return { startCurrent, endCurrent, startPrev, endPrev, periodLabel: 'last year' };
  }

  if (timeframe === 'All Time') {
    const startCurrent = new Date(2020, 0, 1, 0, 0, 0, 0);
    const endCurrent = new Date(year + 5, 11, 31, 23, 59, 59, 999);
    return { startCurrent, endCurrent, startPrev: startCurrent, endPrev: endCurrent, periodLabel: 'all time' };
  }
  ...
  return { startCurrent, endCurrent, startPrev, endPrev, periodLabel: 'last month' };
}

function calculateTrend(current, previous, periodLabel) {
  if (previous === 0) {
    if (current > 0) return { text: `▲ +${current} vs ${periodLabel}`, isUp: true, neutral: false };
    return { text: `0 vs ${periodLabel}`, isUp: false, neutral: true };
  }
  const diff = current - previous;
  const pct = Math.round((diff / previous) * 100);
  if (pct > 0) return { text: `▲ ${pct}% vs ${periodLabel}`, isUp: true, neutral: false };
  if (pct < 0) return { text: `▼ ${Math.abs(pct)}% vs ${periodLabel}`, isUp: false, neutral: false };
  return { text: `0% vs ${periodLabel}`, isUp: false, neutral: true };
}
...
export default function PerformancePage() {
  ...
  const isSalesRole = userRole.includes('sales') && !userRole.includes('manager') && !userRole.includes('admin');
  ...
  useEffect(() => {
    try {
      ...
      const resolvedRole = role || (storedUser.roleSlug || storedUser.role || '').toLowerCase();
      setUserRole(resolvedRole);
      setCurrentUserId(String(storedUser._id || storedUser.id || ''));
      setCurrentUserName(storedUser.name || localStorage.getItem('userName') || 'My Performance');
    } catch {}
  }, []);
  ...
  const loadPerformanceData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      const tfParam = timeframe === 'Quarter to Date' ? 'quarter' : timeframe === 'Year to Date' ? 'all' : 'month';
      ...
      if (isSalesRole) {
        ...
        const [
          ...
          api.get(`/targets/my-achievement?periodStart=${encodeURIComponent(startIso)}&periodEnd=${encodeURIComponent(endIso)}`),
        ]);
        ...
        if (ordersRes.status === 'fulfilled' && ordersRes.value?.data) {
          setOrders(Array.isArray(ordersRes.value.data) ? ordersRes.value.data : (ordersRes.value.data.orders || []));
        }
        if (paymentsRes.status === 'fulfilled' && paymentsRes.value?.data) {
          setPayments(Array.isArray(paymentsRes.value.data) ? paymentsRes.value.data : (paymentsRes.value.data.payments || []));
        }
        ...
        const [
          ...
          api.get('/payments?limit=100'),
          api.get('/followups?limit=250'),
          api.get(`/targets?periodStart=${encodeURIComponent(startIso)}&periodEnd=${encodeURIComponent(endIso)}`),
        ...
        if (usersRes.status === 'fulfilled' && usersRes.value?.data) {
          setUsersList(Array.isArray(usersRes.value.data) ? usersRes.value.data : (usersRes.value.data.users || []));
        }
      }
    } catch (err) {
  ...
  const myTargetRupees = myAchievement?.targetPaise ? myAchievement.targetPaise / 100 : 0;
  const myAchievedRupees = myAchievement?.achievedPaise ? myAchievement.achievedPaise / 100 : 0;
  const myAchievementPct = myAchievement?.achievementPercent || 0;
  const myRemainingRupees = myAchievement?.remainingPaise ? myAchievement.remainingPaise / 100 : 0;
  const myRequiredPerDay = myAchievement?.requiredPerDayPaise ? myAchievement.requiredPerDayPaise / 100 : 0;
  ...
  const leadsTrend = useMemo(() => calculateTrend(totalLeadsCount, prevLeads.length, periodLabel), [totalLeadsCount, prevLeads.length, periodLabel]);

  const convertedLeadsCount = useMemo(() => {
    return currentLeads.filter((l) => ['WON', 'ORDER_CREATED', 'CLOSED_WON'].includes(l.status)).length;
  }, [currentLeads]);
  const prevConvertedCount = useMemo(() => {
    return prevLeads.filter((l) => ['WON', 'ORDER_CREATED', 'CLOSED_WON'].includes(l.status)).length;
  }, [prevLeads]);
  const conversionsTrend = useMemo(() => calculateTrend(convertedLeadsCount, prevConvertedCount, periodLabel), [convertedLeadsCount, prevConvertedCount, periodLabel]);

  const totalOrdersCount = currentOrders.length;
  const ordersTrend = useMemo(() => calculateTrend(totalOrdersCount, prevOrders.length, periodLabel), [totalOrdersCount, prevOrders.length, periodLabel]);

  const totalOrderValueRupees = useMemo(() => {
    return currentOrders.reduce((sum, o) => {
      const val = o.grandTotalPaise ? o.grandTotalPaise / 100 : (o.grandTotal || o.totalAmount || 0);
      return sum + val;
    }, 0);
  }, [currentOrders]);
  ...
  const orderValueTrend = useMemo(() => calculateTrend(totalOrderValueRupees, prevOrderValueRupees, periodLabel), [totalOrderValueRupees, prevOrderValueRupees, periodLabel]);
  ...
  const collectionTrend = useMemo(() => calculateTrend(totalCollectionRupees, prevCollectionRupees, periodLabel), [totalCollectionRupees, prevCollectionRupees, periodLabel]);
  ...
  const executives = useMemo(() => {
      ...
      targetsData.targets.forEach((t) => {
        if (t.user?._id) {
          userMap.set(String(t.user._id), {
            ...
            targetRupees: t.targetRupees || (t.targetPaise ? t.targetPaise / 100 : 0),
    ...
    if (usersList && Array.isArray(usersList)) {
      usersList.forEach((u) => {
        ...
        if (role.includes('sales') || role.includes('rep') || role.includes('executive')) {
    ...
    const repList = Array.from(userMap.values()).map((rep) => {
      ...
      // Conversions
      const repConversions = repLeads.filter((l) => ['WON', 'ORDER_CREATED', 'CLOSED_WON'].includes(l.status)).length;
      const repPrevConversions = repPrevLeads.filter((l) => ['WON', 'ORDER_CREATED', 'CLOSED_WON'].includes(l.status)).length;

      // Confirmed Orders
      const repOrders = currentOrders.filter((o) => {
        const repId = String(o.salesRep?._id || o.salesRep || o.createdById?._id || o.createdById || '');
        return repId === rep.id;
      });
      ...
      const repPayments = currentPayments.filter((p) => {
        const recordedBy = String(p.recordedBy?._id || p.recordedBy || p.userId?._id || p.userId || '');
      ...
      // Conversion rate
      const convRateNum = repLeads.length > 0 ? (repConversions / repLeads.length) * 100 : 0;
      ...
      return {
        ...
        leadsDelta: calculateTrend(repLeads.length, repPrevLeads.length, periodLabel).text,
        conversions: repConversions,
        conversionsDelta: calculateTrend(repConversions, repPrevConversions, periodLabel).text,
        convRate: convRateStr,
        convRateNum,
        orders: repOrders.length,
        ordersDelta: calculateTrend(repOrders.length, repPrevOrders.length, periodLabel).text,
        orderValue: repOrderValue,
        orderValueFormatted: formatINR(repOrderValue),
        orderValueDelta: calculateTrend(repOrderValue, repPrevOrderValue, periodLabel).text,
  ...
  const { totalTargetRupees, overallAchievementPct, teamTargetProgress } = useMemo(() => {
    if (isSalesRole) {
    ...
    const sumTargets = executives.reduce((sum, e) => sum + (e.targetRupees || 0), 0);
  ...
  }, [executives, totalOrderValueRupees, isSalesRole, myTargetRupees, myAchievementPct]);
  ...
  const filteredExecutives = useMemo(() => {
    ...
    return executives.filter(
      (e) => e.name.toLowerCase().includes(q) || e.email.toLowerCase().includes(q) || e.role.toLowerCase().includes(q)
    );
  ...
  const activityMetrics = useMemo(() => {
    ...
    const calls = currentFollowups.filter((f) => f.type === 'CALL' || f.channel === 'CALL').length;
    const whatsapp = currentFollowups.filter((f) => f.type === 'WHATSAPP' || f.channel === 'WHATSAPP').length;
    const meetings = currentFollowups.filter((f) => f.type === 'MEETING' || f.channel === 'MEETING').length;
    const followupsCount = currentFollowups.filter((f) => f.type === 'FOLLOW_UP' || f.type === 'TASK').length;
    const others = Math.max(0, total - (calls + whatsapp + meetings + followupsCount));
  ...
  const funnelMetrics = useMemo(() => {
    ...
    const qualified = currentLeads.filter((l) =>
      ['CONTACTED', 'INTERESTED', 'QUOTATION_SENT', 'NEGOTIATION', 'WON', 'ORDER_CREATED'].includes(l.status)
    ).length;
    const quotationsCount = currentLeads.filter((l) =>
      ['QUOTATION_SENT', 'NEGOTIATION', 'WON', 'ORDER_CREATED'].includes(l.status)
    ).length;
    const wonCount = convertedLeadsCount;

    const qualifiedPct = total > 0 ? ((qualified / total) * 100).toFixed(1) : '0.0';
    const quotationsPct = total > 0 ? ((quotationsCount / total) * 100).toFixed(1) : '0.0';
  ...
  const insights = useMemo(() => {
    ...
    const underperformingCount = executives.filter((e) => e.achievementNum < 80 && e.targetRupees > 0).length;
    const overallConvRate = totalLeadsCount > 0 ? ((convertedLeadsCount / totalLeadsCount) * 100).toFixed(1) : '0.0';
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `performance-report-${timeframe.toLowerCase().replace(/\s+/g, '-')}.csv`);
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="border-b border-slate-200 flex gap-6 overflow-x-auto text-xs font-bold scrollbar-none">
            {TABS.filter((tab) => {
              if (isSalesRole && (tab === 'Executive Performance' || tab === 'Team Performance')) return false;
              return true;
            }).map((tab) => (
              <button
                key={tab}
          ...
          {activeTab === 'Overview' && (
            <div className="space-y-6">
              ...
              {isSalesRole && (
                <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-5 text-white shadow-md shadow-blue-200 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-48 h-48 opacity-[0.06]">
                    <Target className="w-full h-full" />
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative">
                    <div>
                      <p className="text-[11px] font-bold uppercase tracking-widest text-blue-200 mb-0.5">My Target · {timeframe}</p>
                      <div className="text-2xl font-black">
                        {loading ? '...' : formatINR(myTargetRupees)}
                      </div>
                      <p className="text-xs text-blue-200 mt-0.5">
                        Achieved: <span className="text-white font-bold">{loading ? '...' : formatINR(myAchievedRupees)}</span>
                        &nbsp;&middot;&nbsp;Remaining: <span className="text-white font-bold">{loading ? '...' : formatINR(myRemainingRupees)}</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-black">{loading ? '...' : `${myAchievementPct}%`}</div>
                      <p className="text-[11px] text-blue-200 mt-0.5">
                        {myDaysRemaining} days left &middot; Need {loading ? '...' : formatINR(myRequiredPerDay)}/day
                      </p>
                    </div>
                  </div>
                  {/* Progress bar */}
                  <div className="mt-4 relative">
                    ...
                    <div className="flex justify-between text-[10px] text-blue-200 mt-1 font-semibold">
                      <span>₹0</span>
                      <span className="font-bold text-white">{myAchievementPct}% achieved</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-500">Total Leads Assigned</span>
                  ...
                  <div>
                    ...
                    <div className={`text-[10px] font-bold mt-0.5 ${leadsTrend.isUp ? 'text-emerald-600' : leadsTrend.neutral ? 'text-slate-400' : 'text-rose-600'}`}>
                      {leadsTrend.text}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-500">Total Conversions</span>
                  ...
                  <div>
                    ...
                    <div className={`text-[10px] font-bold mt-0.5 ${conversionsTrend.isUp ? 'text-emerald-600' : conversionsTrend.neutral ? 'text-slate-400' : 'text-rose-600'}`}>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <div className={`text-[10px] font-bold mt-0.5 ${ordersTrend.isUp ? 'text-emerald-600' : ordersTrend.neutral ? 'text-slate-400' : 'text-rose-600'}`}>
                      {ordersTrend.text}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-500">Total Order Value</span>
                  ...
                  <div>
                    ...
                    <div className={`text-[10px] font-bold mt-0.5 ${orderValueTrend.isUp ? 'text-emerald-600' : orderValueTrend.neutral ? 'text-slate-400' : 'text-rose-600'}`}>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-500">Collection (Received)</span>
                  ...
                  <div>
                    ...
                    <div className={`text-[10px] font-bold mt-0.5 ${collectionTrend.isUp ? 'text-emerald-600' : collectionTrend.neutral ? 'text-slate-400' : 'text-rose-600'}`}>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-500">Overall Achievement</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-8 bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                    <div>
                      <h3 className="text-sm font-bold text-slate-900">Individual Performance</h3>
                      <p className="text-[11px] text-slate-400">
                        Performance of team members for {timeframe.toLowerCase()}
                      </p>
                    </div>
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {loading ? (
                          ...
                          filteredExecutives.map((exec) => (
                            <tr key={exec.id || exec.rank} className="hover:bg-slate-50/80 transition-colors">
                              <td className="py-3.5 font-bold text-slate-400">{exec.rank}</td>
                              <td className="py-3.5">
                                <div className="flex items-center gap-2">
                                  <div
                                  ...
                                  <div className="min-w-0 max-w-[140px]">
                                    <p className="font-bold text-slate-900 leading-tight truncate">{exec.name}</p>
                                    <p className="text-[10px] text-slate-400 truncate">{exec.role}</p>
                                  </div>
                                </div>
                              </td>
                              <td className="py-3.5 text-center">
                                <span className="font-bold text-slate-800">{exec.leads}</span>
                                <span className="block text-[9px] font-bold text-slate-400">{exec.leadsDelta}</span>
                              </td>
                              <td className="py-3.5 text-center">
                                <span className="font-bold text-slate-800">{exec.conversions}</span>
                                <span className="block text-[9px] font-bold text-slate-400">{exec.conversionsDelta}</span>
                              </td>
                              <td className="py-3.5 text-center font-bold text-slate-800">
                                {exec.convRate}
                              </td>
                              <td className="py-3.5 text-center">
                                <span className="font-bold text-slate-800">{exec.orders}</span>
                                <span className="block text-[9px] font-bold text-slate-400">{exec.ordersDelta}</span>
                              </td>
                              ...
                              <td className="py-3.5">
                                <span className="font-bold text-slate-800 text-xs">{exec.achievement}</span>
                                <div className="w-16 h-1.5 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                  <div
                                    className={`h-full ${exec.barColor} rounded-full transition-all duration-500`}
                                    style={{ width: `${Math.min(exec.achievementNum, 100)}%` }}
                                  />
                                </div>
                              </td>
                      ...
                      <tfoot>
                        <tr className="border-t-2 border-slate-200 font-bold text-slate-900 text-xs">
                          <td colSpan={2} className="pt-3">Total / Average</td>
                          <td className="pt-3 text-center">{totalLeadsCount}</td>
                          <td className="pt-3 text-center">{convertedLeadsCount}</td>
                          <td className="pt-3 text-center">
                            {totalLeadsCount > 0 ? `${((convertedLeadsCount / totalLeadsCount) * 100).toFixed(1)}%` : '0%'}
                          </td>
                          <td className="pt-3 text-center">{totalOrdersCount}</td>
                          <td className="pt-3">{formatINR(totalOrderValueRupees)}</td>
                          <td className="pt-3">{formatINR(totalCollectionRupees)}</td>
                ...
                <div className="lg:col-span-4 space-y-4">
                  ...
                  <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-bold text-slate-900">Target vs Achievement</h3>
                      <Link
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="w-24 h-24 relative flex items-center justify-center shrink-0">
                        ...
                        <div className="absolute flex flex-col items-center">
                          ...
                          <span className="text-[8px] text-slate-400 font-medium mt-0.5">Overall Ach.</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 text-xs flex-1">
                        <div className="flex justify-between text-slate-500">
                          ...
                          <span className="font-bold text-slate-900 truncate max-w-[110px]">
                            {totalTargetRupees > 0 ? formatINR(totalTargetRupees) : 'Not Set'}
                          </span>
                        </div>
                        <div className="flex justify-between text-slate-500">
                          <span>Achieved:</span>
                  ...
                  <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-bold text-slate-900">Performance by Activity</h3>
                      <Link href="/dashboard/followups" className="text-[10px] font-bold text-blue-600 hover:underline">
                        View Activities
                      </Link>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-24 h-24 relative flex items-center justify-center shrink-0">
                        ...
                        <div className="absolute flex flex-col items-center">
                          ...
                          <span className="text-[7px] text-slate-400 font-medium mt-0.5">Total Acts</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 text-[11px] flex-1">
                        <div className="flex items-center justify-between">
                          <span className="flex items-center gap-1.5 text-slate-600">
                            <span className="w-2 h-2 rounded-full bg-blue-500" /> Calls
                          </span>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-bold text-slate-800">
                            {activityMetrics.whatsappPct}% ({activityMetrics.whatsapp})
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="flex items-center gap-1.5 text-slate-600">
                          ...
                          <span className="font-bold text-slate-800">
                            {activityMetrics.meetingsPct}% ({activityMetrics.meetings})
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="flex items-center gap-1.5 text-slate-600">
                            <span className="w-2 h-2 rounded-full bg-rose-500" /> Follow-ups
                          </span>
                          <span className="font-bold text-slate-800">
                            {activityMetrics.followupsPct}% ({activityMetrics.followupsCount})
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-6 bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xs font-bold text-slate-900">Performance Trend</h3>
                      <div className="flex items-center gap-4 text-[10px] font-bold">
                        <span className="flex items-center gap-1 text-blue-600">
                          <span className="w-2 h-2 rounded-full bg-blue-500" /> Leads ({totalLeadsCount})
                        </span>
                        <span className="flex items-center gap-1 text-emerald-600">
                          <span className="w-2 h-2 rounded-full bg-emerald-500" /> Orders ({totalOrdersCount})
                        </span>
                        <span className="flex items-center gap-1 text-purple-600">
                          <span className="w-2 h-2 rounded-full bg-purple-500" /> Collections ({formatINR(totalCollectionRupees)})
                        </span>
                      </div>
                    </div>
                ...
                <div className="lg:col-span-3 bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <h3 className="text-xs font-bold text-slate-900 mb-2">Conversion Funnel</h3>
                  ...
                  <div className="space-y-1.5 text-xs font-semibold py-2">
                    <div className="p-2 rounded-xl bg-blue-500 text-white text-center shadow-xs">
                      <div className="text-xs font-black">{funnelMetrics.total}</div>
                    ...
                    <div className="p-2 rounded-xl bg-emerald-500 text-white text-center shadow-xs mx-2">
                      ...
                      <div className="text-[9px] opacity-80">Qualified / In Progress</div>
                    </div>
                    <div className="p-2 rounded-xl bg-purple-500 text-white text-center shadow-xs mx-4">
                      <div className="text-xs font-black">
                        {funnelMetrics.quotationsCount} ({funnelMetrics.quotationsPct}%)
                      </div>
                      <div className="text-[9px] opacity-80">Quotations / Negotiations</div>
                    </div>
                  ...
                  <div className="text-center text-[11px] text-slate-400 pt-2 border-t border-slate-100">
                    ...
                    <span className="text-emerald-600 font-bold">{funnelMetrics.throughput}%</span>
                  </div>
                </div>
                ...
                <div className="lg:col-span-3 bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                  <div>
                    <h3 className="text-xs font-bold text-slate-900 mb-3">Top Performers ({timeframe})</h3>
                      ...
                      {topPerformers.length === 0 ? (
                        ...
                        topPerformers.map((p) => (
                          <div key={p.rank} className="flex items-center justify-between text-xs py-1">
                            ...
                            <div className="text-right">
                              <span className="font-bold text-emerald-600 block leading-tight">{p.achievement}</span>
                              <span className="text-[10px] text-slate-400">{p.orderValueFormatted}</span>
                            </div>
                          </div>
                        ))
                      )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {insights.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`p-3.5 rounded-2xl ${item.bg} border ${item.border} flex items-center gap-3 transition-all`}
                    >
                      <IconComp className={`w-5 h-5 ${item.iconColor} shrink-0`} />
                      <div className="min-w-0 flex-1">
                        <p className={`text-xs font-bold ${item.textColor} truncate`}>{item.title}</p>
                        <p className={`text-[11px] ${item.descColor} line-clamp-2`}>{item.text}</p>
                      </div>
                    </div>
                  );
                })}
          ...
          {activeTab === 'Executive Performance' && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-base font-bold text-slate-900">Executive Directory & Metric Cards</h2>
                  <p className="text-xs text-slate-500">
                    Comprehensive quota attainment and sales activities per representative
                  </p>
                </div>
              ...
              {filteredExecutives.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 border border-slate-200/90 text-center text-slate-400">
                  <Inbox className="w-8 h-8 mx-auto mb-2 text-slate-300" />
                  <p className="text-xs font-bold text-slate-600">No representatives match your search query</p>
                  <p className="text-[11px] text-slate-400 mt-1">Try searching for a different name or role</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredExecutives.map((exec) => (
                    <div
                      ...
                      className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-all space-y-4"
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h3 className="font-bold text-slate-900 text-sm truncate">{exec.name}</h3>
                            <p className="text-[11px] text-slate-400 truncate">{exec.email || exec.role}</p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 py-2 border-y border-slate-100 text-center">
                        <div>
                          ...
                          <strong className="text-slate-900 text-sm">{exec.leads}</strong>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 block font-semibold">Orders</span>
                          <strong className="text-slate-900 text-sm">{exec.orders}</strong>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 block font-semibold">Win Rate</span>
                          <strong className="text-emerald-600 text-sm">{exec.convRate}</strong>
                        </div>
                      </div>

                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-semibold">Revenue Achieved:</span>
                          <strong className="text-slate-900">{exec.orderValueFormatted}</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-semibold">Target Quota:</span>
                          <strong className="text-slate-700">
                            {exec.targetRupees > 0 ? formatINR(exec.targetRupees) : 'Not Configured'}
                          </strong>
                        </div>
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-slate-500 pt-1">
                          ...
                          <span className="text-emerald-700">★ {exec.rating} Rating</span>
                        </div>
                      </div>

                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                        <span className="text-slate-500 text-[11px]">Collections: {exec.collectionFormatted}</span>
                        <Link
          ...
          {activeTab === 'Team Performance' && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-slate-900 to-blue-950 p-6 rounded-2xl text-white shadow-md space-y-4">
                  ...
                  <div>
                    ...
                    <h2 className="text-2xl font-black mt-1">
                      ...
                      <span className="text-sm font-normal text-slate-300">
                        / {totalTargetRupees > 0 ? formatINR(totalTargetRupees) : 'No team cap set'}
                      </span>
                    </h2>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <div className="text-3xl font-black text-amber-300">{overallAchievementPct}%</div>
                      <div className="text-[10px] text-blue-200">Team Target Attainment</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 border-t border-white/10 text-xs">
                  <div>
                    <span className="text-blue-300 block text-[11px]">Active Executives</span>
                    <strong className="text-base font-bold">{executives.length}</strong>
                  </div>
                  <div>
                    <span className="text-blue-300 block text-[11px]">Total Won Orders</span>
                    <strong className="text-base font-bold">{totalOrdersCount}</strong>
                  </div>
                  <div>
                    <span className="text-blue-300 block text-[11px]">Total Pipeline Leads</span>
                    <strong className="text-base font-bold">{totalLeadsCount}</strong>
                  </div>
                  <div>
                    <span className="text-blue-300 block text-[11px]">Team Win Rate</span>
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-xs space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900">Comparative Revenue by Representative</h3>
                  <span className="text-xs font-semibold text-slate-400">{timeframe}</span>
                </div>

                <div className="space-y-3 pt-2">
                  {executives.map((exec) => {
                    const maxVal = Math.max(...executives.map((e) => e.orderValue), 1);
                    const widthPct = Math.max(5, Math.round((exec.orderValue / maxVal) * 100));

                    return (
                      <div key={exec.id || exec.rank} className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="font-bold text-slate-800">{exec.name}</span>
                          <span className="font-bold text-slate-900">{exec.orderValueFormatted}</span>
                        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs">
                  ...
                  <div className="text-2xl font-black text-slate-900">{activityMetrics.total}</div>
                  <span className="text-[10px] text-slate-400">Logged interactions</span>
                </div>

                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-slate-500">Phone Calls</span>
                    <PhoneCall className="w-4 h-4 text-emerald-600" />
                  </div>
                  <div className="text-2xl font-black text-slate-900">{activityMetrics.calls}</div>
                  <span className="text-[10px] text-emerald-600 font-bold">{activityMetrics.callsPct}% of volume</span>
                </div>

                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-slate-500">WhatsApp Chats</span>
                    <MessageSquare className="w-4 h-4 text-purple-600" />
                  </div>
                  <div className="text-2xl font-black text-slate-900">{activityMetrics.whatsapp}</div>
                  <span className="text-[10px] text-purple-600 font-bold">{activityMetrics.whatsappPct}% of volume</span>
                </div>
                ...
                <div className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs">
                  ...
                  <div className="text-2xl font-black text-slate-900">{activityMetrics.meetings}</div>
                  <span className="text-[10px] text-amber-600 font-bold">{activityMetrics.meetingsPct}% of volume</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">Activity Log Pacing</h3>
                    <p className="text-xs text-slate-500">Recent follow-up actions recorded in this timeframe</p>
                  </div>
                  <Link
                ...
                <div className="divide-y divide-slate-100">
                  {currentFollowups.slice(0, 10).map((act, idx) => (
                    <div key={act._id || idx} className="py-3 flex items-center justify-between text-xs gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <strong className="text-slate-900 block truncate">
                            {act.notes || act.title || `Follow-up on ${act.channel || act.type || 'Lead'}`}
                          </strong>
                          <span className="text-[10px] text-slate-400">
                            {act.scheduledAt ? new Date(act.scheduledAt).toLocaleDateString('en-IN') : 'Scheduled'}
                          </span>
                        </div>
                      </div>
                      <span
          ...
          {activeTab === 'Targets vs Achievement' && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-base font-bold text-slate-900">Commercial Target Matrix</h2>
                  <p className="text-xs text-slate-500">
                    Real-time sales target tracking derived strictly from confirmed won orders
                  </p>
                </div>
                <Link
                  href="/dashboard/sales-target"
              ...
              <div className="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-100 text-slate-400 text-[10px] uppercase font-bold tracking-wider">
                      <th className="pb-3">Executive</th>
                    ...
                    {executives.map((exec) => (
                      <tr key={exec.id || exec.rank} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3.5 font-bold text-slate-700">
                          {exec.targetRupees > 0 ? formatINR(exec.targetRupees) : 'Not Set'}
                        </td>
                        <td className="py-3.5 font-bold text-emerald-600">{exec.orderValueFormatted}</td>
                        <td className="py-3.5 text-slate-600">
                          {exec.targetRupees > 0
                            ? formatINR(Math.max(0, exec.targetRupees - exec.orderValue))
                            : '₹0'}
                        </td>
                        <td className="py-3.5 text-slate-700">{exec.targetOrdersCount || '-'}</td>
                        <td className="py-3.5 font-bold text-slate-900">{exec.orders}</td>
                        <td className="py-3.5">
                          <span className="font-bold text-slate-900 block">{exec.achievement}</span>
                        ...
                        <td className="py-3.5 text-right font-mono font-bold text-slate-700">
                          {exec.requiredPerDayRupees > 0 ? `${formatINR(exec.requiredPerDayRupees)}/day` : '-'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
