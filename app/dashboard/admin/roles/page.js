...
const DEFAULT_STANDARD_ROLES = [
  {
    ...
    description:
      "Unrestricted enterprise control and strategic oversight across all company resources.",
  ...
  },
  {
    ...
    description:
      "Team management, lead distribution, quotation approvals, and performance oversight.",
    isSystemDefault: true,
    permissions: [
  ...
  },
  {
    ...
    description:
      "Client communication, quotation preparation, order intake, and follow-up management.",
    isSystemDefault: true,
    permissions: [
...
export default function RolesPermissionsPage() {
  ...
  const fetchRoles = async () => {
    try {
      ...
      console.error(
        "Failed to fetch roles from server, preserving canonical roles:",
        err,
      );
    } finally {
      setLoading(false);
  ...
  const getPermissionFor = (resourceId, action) => {
    ...
    const directMatch = activeRole.permissions.find(
      (p) =>
        (p.resource?.toUpperCase() === resourceId.toUpperCase() ||
          p.resource === "*") &&
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="p-8 max-w-2xl mx-auto w-full text-center space-y-4 my-auto">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Roles and permissions policy governance is restricted to System
              Administrators.
            </p>
          </div>
        </main>
    ...
    <div className="flex bg-[#F8FAFC] min-h-screen text-slate-800 font-sans antialiased">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Live role governance and data scope enforcement (ALL · AREA ·
                TEAM · OWN · NONE)
              </p>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-500 mt-1 line-clamp-1">
                    {roleInfo?.description || "System Role"}
                  </p>
                </button>
              );
            })}
          ...
          <div className="p-4 rounded-md bg-amber-50 border border-amber-200 text-amber-900 flex items-start gap-3 text-xs shadow-xs">
            <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div>
              <span className="font-bold block mb-0.5">
                Role Governance & Safety Rails Active
              </span>
              <p className="text-[11px] text-amber-800 leading-relaxed">
                System default roles are enforced by server-side middleware
                (`tenantContext` & `authorize`). Granting `OVERRIDE` or changing
                dataScopes to `ALL` requires elevated administrative
                authorization.
              </p>
            </div>
          </div>
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-[10px] uppercase font-bold tracking-wider">
                    ...
                    {ACTIONS.map((action) => (
                      <th key={action} className="py-3 px-3 text-center">
                        {action}
                      </th>
                    ))}
                  </tr>
                      ...
                      {ACTIONS.map((action) => {
                        ...
                        return (
                          <td key={action} className="py-3.5 px-3 text-center">
                            {isGranted ? (
                              <span
                                className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] border shadow-2xs ${getScopeBadgeColor(scope)}`}
                              >
                                {scope}
                              </span>
                            ) : (
