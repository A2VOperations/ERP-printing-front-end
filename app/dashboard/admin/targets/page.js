...
export default function AdminTargetsPage() {
  ...
  const loadTargets = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get(`/targets?year=${selectedYear}&month=${selectedMonth}`);
      if (res && res.data) {
        const rawList = res.data.targets || [];
        const salesTargets = rawList.filter((t) => {
          const role = String(t.user?.roleSlug || t.user?.role || '').toLowerCase();
  ...
  const summary = useMemo(() => {
    ...
    const totalTarget = targetsData.reduce((sum, t) => sum + (t.targetRupees || 0), 0);
    const totalAchieved = targetsData.reduce((sum, t) => sum + (t.achievedRupees || 0), 0);
    const totalOrdersWon = targetsData.reduce((sum, t) => sum + (t.ordersWonCount || 0), 0);
    const totalTargetOrders = targetsData.reduce((sum, t) => sum + (t.targetOrdersCount || 0), 0);
    const progressPercent = totalTarget > 0 ? ((totalAchieved / totalTarget) * 100).toFixed(1) : 0;
  ...
  const handleOpenEdit = (item) => {
    ...
    setTargetForm({
      ...
      targetOrdersCount: item.targetOrdersCount > 0 ? item.targetOrdersCount : 10,
  ...
  const handleOpenCreate = () => {
    ...
    const firstRepWithoutTarget = targetsData.find((t) => !t.hasTarget)?.user || usersList[0];
    setTargetForm({
  ...
  const handleSaveTarget = async (e) => {
    ...
    try {
      setSaving(true);
      const pStart = new Date(selectedYear, selectedMonth, 1, 0, 0, 0, 0).toISOString();
      const pEnd = new Date(selectedYear, selectedMonth + 1, 0, 23, 59, 59, 999).toISOString();
      ...
      if (modalMode === 'BATCH') {
        ...
        showNotification(`Applied targets to all ${usersList.length} sales representatives!`);
      } else {
        ...
        showNotification(`Target successfully configured for ${selectedUser?.name || 'Representative'}!`);
      }
  ...
  const handleDeleteTarget = async (item) => {
    ...
    if (!window.confirm(`Are you sure you want to remove the target for ${item.user.name}?`)) return;
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    Configure individual and team revenue goals, track closed sales, and govern live sales incentives.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xl font-black text-slate-900">
                  ₹{summary.totalTarget.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </span>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xl font-black text-emerald-600">
                  ₹{summary.totalAchieved.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </span>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
              ...
              <span className="text-[11px] text-slate-500 block">
                {summary.totalReps - summary.repsWithTarget} reps awaiting targets
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-slate-100 rounded-xl p-1 text-xs font-bold text-slate-600">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg transition-colors ${
                      filterStatus === 'SET' ? 'bg-white text-emerald-700 shadow-2xs' : 'hover:text-slate-900'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg transition-colors ${
                      filterStatus === 'NOT_SET' ? 'bg-white text-amber-700 shadow-2xs' : 'hover:text-slate-900'
                    }`}
                  >
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <div className="py-16 text-center text-slate-400 text-xs space-y-2">
                  ...
                  <p className="font-bold text-slate-700">No representatives found</p>
                        ...
                        <tr key={rep._id} className="hover:bg-slate-50/80 transition-colors group">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span className="text-[11px] text-slate-400 block font-medium">
                                  {rep.email} · <span className="capitalize">{rep.role || 'Sales Rep'}</span>
                                </span>
                              </div>
                            </div>
                          </td>
                            ...
                            {hasTarget ? (
                              <div>
                                <span className="font-black text-slate-900 text-sm block">
                                  ₹{item.targetRupees.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                                </span>
                          ...
                          <td className="py-3.5">
                            <span className="font-black text-emerald-600 text-sm block">
                              ₹{item.achievedRupees.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                            </span>
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between text-[11px]">
                                ...
                                <span className="text-[10px] text-slate-400">
                                  {hasTarget ? `₹${item.remainingRupees.toLocaleString('en-IN', { maximumFractionDigits: 0 })} left` : '-'}
                                </span>
                              </div>
                              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-500 ${
                                    pct >= 100 ? 'bg-emerald-500' : pct >= 50 ? 'bg-blue-600' : pct > 0 ? 'bg-amber-500' : 'bg-slate-200'
                                  }`}
                            ...
                            {hasTarget && item.remainingRupees > 0 ? (
                              <div>
                                <span className="font-bold text-slate-800 text-xs block">
                                  ₹{item.requiredPerDayRupees.toLocaleString('en-IN', { maximumFractionDigits: 0 })} / day
                                </span>
                                <span className="text-[10px] text-slate-400">
                          ...
                          <td className="py-3.5 max-w-[180px]">
                            <span className="text-[11px] text-slate-500 block truncate" title={item.notes || 'No remarks'}>
            ...
            <form onSubmit={handleSaveTarget} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedUser(u);
                      setTargetForm((prev) => ({ ...prev, userId: e.target.value }));
                    }}
                    required
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setTargetForm((prev) => ({ ...prev, targetAmount: e.target.value }))}
                ...
                <div className="flex items-center gap-1.5 flex-wrap pt-1">
                  ...
                  {[50000, 100000, 200000, 500000, 1000000].map((amt) => (
                    <button
                      type="button"
                      key={amt}
                      onClick={() => setTargetForm((prev) => ({ ...prev, targetAmount: amt }))}
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setTargetForm((prev) => ({ ...prev, targetOrdersCount: e.target.value }))}
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  onChange={(e) => setTargetForm((prev) => ({ ...prev, notes: e.target.value }))}
