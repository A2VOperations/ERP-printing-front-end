        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="p-8 max-w-2xl mx-auto w-full text-center space-y-4 my-auto">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              System business rules configuration is restricted to System Administrators.
            </p>
          </div>
        </main>
      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Canonical business constraints for CRM pipeline, BPS discount governance, and Phase 4 prepress standards
              </p>
            </div>
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-xs space-y-6 text-xs">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Follow-up Grace Window (Hours)</label>
                  <input
                    type="number"
                    value={systemRules.followUpGraceHours}
                    onChange={(e) => setSystemRules({ ...systemRules, followUpGraceHours: Number(e.target.value) })}
                  ...
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Time before due follow-up marks as OVERDUE</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Auto-Approved Max Discount %</label>
                  <input
                    type="number"
                    value={systemRules.maxSalesDiscountPercent}
                    onChange={(e) => setSystemRules({ ...systemRules, maxSalesDiscountPercent: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Discounts $\le$ this rate require no manager sign-off</span>
                </div>

                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Manager Approval Limit %</label>
                  <input
                    type="number"
                    value={systemRules.managerApprovalThresholdPercent}
                    onChange={(e) => setSystemRules({ ...systemRules, managerApprovalThresholdPercent: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Discounts above this escalate to CEO / Super Admin</span>
                </div>

                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Default Standard GST %</label>
                  <input
                    type="number"
                    value={systemRules.defaultGstPercent}
                    onChange={(e) => setSystemRules({ ...systemRules, defaultGstPercent: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Intrastate CGST (9%) + SGST (9%) or IGST (18%)</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Mandatory Advance Percentage %</label>
                  <input
                    type="number"
                    value={systemRules.mandatoryAdvancePercent}
                    onChange={(e) => setSystemRules({ ...systemRules, mandatoryAdvancePercent: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Minimum payment to trigger commercialReady clearance</span>
                </div>

                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Receivables Ageing Grace Days</label>
                  <input
                    type="number"
                    value={systemRules.creditTermGraceDays}
                    onChange={(e) => setSystemRules({ ...systemRules, creditTermGraceDays: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Grace period before balance moves to OVERDUE bucket</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Included Proof Revisions</label>
                  <input
                    type="number"
                    value={systemRules.includedRevisionLimit}
                    onChange={(e) => setSystemRules({ ...systemRules, includedRevisionLimit: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Free client revision cycles per design project</span>
                </div>

                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Minimum Preflight DPI</label>
                  <input
                    type="number"
                    value={systemRules.minPreflightDpi}
                    onChange={(e) => setSystemRules({ ...systemRules, minPreflightDpi: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Resolution threshold for preflight verification PASS</span>
                </div>

                <div>
                  <label className="text-slate-700 font-semibold block mb-1">Dimension Tolerance (mm)</label>
                  <input
                    type="number"
                    value={systemRules.dimensionToleranceMm}
                    onChange={(e) => setSystemRules({ ...systemRules, dimensionToleranceMm: Number(e.target.value) })}
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:outline-none focus:border-blue-500 font-bold"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Allowed bleed variance before preflight WARNING</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              ...
              >
                ...
                {saving ? 'Updating System Rules...' : 'Save System Business Rules'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-sm p-6 space-y-4 shadow-2xl animate-scale-up text-center">
            ...
            <h3 className="text-base font-bold text-slate-900">Update System Business Rules?</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              These rules directly affect commercial discounts, mandatory advance requirements, and preflight criteria system-wide.
            </p>
