...
export default function UsersDirectoryPage() {
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      if (
        selectedUser.newRoleSlug &&
        selectedUser.newRoleSlug !== selectedUser.roleSlug
      ) {
        const matchingRole = roles.find(
          (r) => r.slug === selectedUser.newRoleSlug,
        );
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesRole =
      roleFilter === "ALL" || (u.roleSlug || u.role) === roleFilter;
                ...
                <tbody className="divide-y divide-slate-100 font-medium">
                  {filteredUsers.length > 0 ? (
                          ...
                          <td className="py-3.5">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {u.hasCustomPermissions && (
                                <span
                                  className="px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-indigo-50 text-indigo-700 border border-indigo-200"
                                  title="Custom Permissions Configured"
                                >
                                  Custom
                                </span>
                              )}
                            ...
                            <div className="flex items-center justify-end gap-2">
                              <button
                                onClick={() => {
                                  setSelectedUser({
                                    ...u,
                                    areaIds: (u.areaIds || []).map((a) =>
                                      typeof a === "object" ? a._id : a,
                                    ),
                                    newRoleSlug:
                                      u.roleSlug || u.role || "sales",
                                    newPassword: "",
                                  });
                    ...
                    <tr>
                      ...
                      >
                        No users matching filter criteria. Click &quot;Create
                        New User&quot; to add a team member.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-scale-up">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-3.5 text-xs">
              ...
              <div>
                ...
                <select
                  value={
                    Array.isArray(selectedUser.areaIds) &&
                    selectedUser.areaIds.length > 0
      ...
      {showConfirmModal && confirmAction && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-sm p-6 space-y-4 shadow-2xl animate-scale-up text-center">
            <div className="w-12 h-12 rounded-md bg-amber-100 text-amber-600 flex items-center justify-center mx-auto">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl animate-scale-up">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {permissionsUser.email} · Assigned Role:{" "}
            ...
            <div className="p-5 space-y-4 overflow-y-auto flex-1 text-xs">
              {loadingPermissions ? (
                ...
                <>
                  {/* Mode Selector */}
                  <div className="p-4 rounded-md bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                      <span className="font-bold text-slate-900 block text-sm">
                        Permission Policy Mode
                      </span>
                      <span className="text-[11px] text-slate-500">
                        Choose whether this user inherits permissions from their
                        assigned role or uses customized individual permissions.
                      </span>
                    </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (
                            ...
                            setCustomPolicies(
                              JSON.parse(JSON.stringify(rolePolicies)),
                            );
                          }
                        }}
                        className={`px-3.5 py-1.5 rounded-lg font-bold text-xs transition-all ${
                  ...
                  {!hasCustomPermissions ? (
                    <div className="p-4 rounded-md bg-blue-50/70 border border-blue-100 text-blue-900 space-y-2">
                      ...
                      <p className="text-[11px] text-blue-700 leading-relaxed">
                        ...
                        made to the &quot;
                        {permissionsUser.roleSlug || permissionsUser.role}&quot;
                        role. Click &quot;Custom Override&quot; above if you
                        want to grant or restrict specific modules for this
                        single user.
                      </p>
                    </div>
                  ) : (
                          ...
                          <table className="w-full text-left border-collapse text-[11px]">
                            <thead className="sticky top-0 bg-slate-100 text-slate-700 font-bold border-b border-slate-200 z-10">
                              <tr>
                                ...
                                <th className="p-2.5 pr-4 text-center">
                                  Data Scope
                                </th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 font-medium">
                              {PERMISSION_RESOURCES.map((res) => {
                                const resourcePolicies = customPolicies.filter(
                                  (p) => p.resource === res.id,
                                );
                                const currentScope =
                                  resourcePolicies[0]?.dataScope || "OWN";
                                      ...
                                      return (
                                        ...
                                        >
                                          <input
                                            ...
                                            checked={isChecked}
                                            onChange={() =>
                                              handleTogglePolicy(res.id, action)
                                            }
