...
export default function ProductionDashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (jobsRes.status === 'fulfilled') {
        const jList = jobsRes.value?.data || (Array.isArray(jobsRes.value) ? jobsRes.value : []);
        setJobs(jList);
      }
  ...
  const openHandoffModal = async () => {
    ...
    try {
      ...
      const eligible = orderList.filter(
        (o) => !['CANCELLED', 'REJECTED', 'DELIVERED', 'COMPLETED'].includes(o.orderStatus)
      );
  ...
  const handleCreateHandoff = async (e) => {
    ...
    try {
      ...
      setHandoffError(err.message || 'Failed to initialize production job from handoff');
    } finally {
    ...
    return jobs.filter((job) => {
      ...
      if (activeTab !== 'ALL') {
        if (activeTab === 'DELIVERED') {
          if (!['DELIVERED', 'COMPLETED'].includes(job.productionStatus)) return false;
  ...
  const mReady = metrics?.readyForRelease ?? jobs.filter((j) => j.productionStatus === 'READY_FOR_RELEASE').length;
  const mSent = metrics?.sentForProduction ?? jobs.filter((j) => j.productionStatus === 'SENT_FOR_PRODUCTION').length;
  const mInProd = metrics?.inProduction ?? jobs.filter((j) => j.productionStatus === 'IN_PRODUCTION').length;
  const mReadyDisp = metrics?.readyForDispatch ?? jobs.filter((j) => j.productionStatus === 'READY_FOR_DISPATCH').length;
  const mDispatched = metrics?.dispatched ?? jobs.filter((j) => j.productionStatus === 'DISPATCHED').length;
  const mDelivered = metrics?.deliveredToday ?? jobs.filter((j) => ['DELIVERED', 'COMPLETED'].includes(j.productionStatus)).length;
  const mOverdue = metrics?.overdue ?? jobs.filter((j) => j.dueDate && new Date(j.dueDate) < new Date() && !['DELIVERED', 'COMPLETED', 'CANCELLED'].includes(j.productionStatus)).length;
    ...
    <div className="flex bg-[#F8FAFC] min-h-screen text-slate-800 font-sans antialiased">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl w-full mx-auto">
          ...
          {/* Role Access Notice */}
          {isViewOnly && (
            <div className="p-3.5 rounded-2xl bg-blue-50 border border-blue-200 flex items-center justify-between text-xs text-blue-800">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Ready Release</span>
            ...
            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Ready Dispatch</span>
              <p className="text-xl font-black text-emerald-600 mt-1">{mReadyDisp}</p>
              <span className="text-[9px] text-slate-400">Printed &amp; received</span>
            </div>

            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Dispatched</span>
              <p className="text-xl font-black text-cyan-600 mt-1">{mDispatched}</p>
              <span className="text-[9px] text-slate-400">In courier transit</span>
            </div>

            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Delivered</span>
              <p className="text-xl font-black text-emerald-700 mt-1">{mDelivered}</p>
              <span className="text-[9px] text-slate-400">POD confirmed</span>
            </div>

            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-rose-600 block">Overdue</span>
              <p className="text-xl font-black text-rose-600 mt-1">{mOverdue}</p>
              <span className="text-[9px] text-slate-400">Past target date</span>
            </div>

            <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <span className="text-[10px] font-bold uppercase tracking-wider text-rose-600 block">Failed Delivery</span>
            ...
            <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-xs font-bold text-rose-900">
                  Attention Queue: {urgentAttentionJobs.length} Urgent / Overdue Jobs
                </h4>
                <p className="text-xs text-rose-700 mt-0.5">
                  The following production jobs require immediate manager review or artisan follow-up:
                </p>
                <div className="flex flex-wrap gap-2 mt-2">
                  {urgentAttentionJobs.slice(0, 4).map((j) => (
                    ...
                    >
                      ...
                      <span className="text-rose-600 font-normal">({j.productionStatus})</span>
                    ...
                    <tr>
                      <td colSpan="7" className="py-12 text-center text-slate-500">
                        ...
                        <p className="text-[11px] text-slate-500 mt-1">
                          Jobs initialize upon commercial approval and locked design assets.
                        </p>
                        {canManage && (
                          <button
                            onClick={openHandoffModal}
                    ...
                    filteredJobs.map((job) => {
                      ...
                      return (
                        <tr key={job._id} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-3 px-4">{getStatusBadge(job.productionStatus)}</td>
                          <td className="py-3 px-4">
                            {job.releaseMethod ? (
                              <div className="space-y-0.5">
                                <span className="text-[11px] font-semibold text-slate-800 flex items-center gap-1">
                                  {job.releaseMethod === 'EMAIL' ? '✉️ Email Sent' : '📋 Manual'}
                                </span>
                                {job.releaseSentAt && (
                              ...
                              <span className="text-slate-400 italic text-[11px]">Pending Release</span>
                            )}
                          </td>
      ...
      {showHandoffModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-xl text-slate-800 animate-scale-up">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">Initialize Production Job</h3>
                <p className="text-xs text-slate-500">Convert approved commercial order into outsourced job</p>
              </div>
              <button
              ...
              <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs space-y-1.5">
                ...
                {handoffError.toLowerCase().includes('lock') && (
                  <p className="text-[11px] text-slate-600">
                    Locked artwork is mandatory before production release. Please open{' '}
                    <Link href="/dashboard/design" className="text-blue-600 underline font-semibold">
                      Design Studio
                    </Link>{' '}
            ...
            <form onSubmit={handleCreateHandoff} className="space-y-4">
              <div>
                ...
                {loadingOrders ? (
                  ...
                  <p className="text-xs text-slate-500 p-3 bg-slate-50 rounded-xl border border-slate-200">
                    No active orders found for production handoff. Ensure you have active commercial orders with locked design artwork.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {ordersAwaitingProduction.map((order) => {
                      ...
                      const custName =
                        ...
                        'Customer';
                      const balance = ((order.balancePaise || 0) / 100).toFixed(2);
                      return (
                        <div
                          key={order._id}
                        ...
                        >
                          <div className="flex justify-between items-center font-bold">
                            <span className="text-sm text-blue-700">{order.orderNumber}</span>
                            <span className="text-[10px] px-2 py-0.5 rounded bg-blue-50 border border-blue-200 text-blue-700">
                              {order.orderStatus}
                            </span>
                          </div>
                          <p className="text-slate-900 text-xs font-semibold mt-1">Client: {custName}</p>
                          <div className="flex justify-between items-center text-slate-500 text-[11px] mt-1">
                            <span>
                              {order.items?.length || 1} item(s) • ₹
                              {((order.grandTotalPaise || 0) / 100).toLocaleString('en-IN')}
                            </span>
                            <span
                              className={
                            ...
                            >
                              {order.balancePaise > 0 ? `₹${balance} due` : 'Fully Cleared'}
                            </span>
                          </div>
                        </div>
                      );
                      ...
                      >
                        {selectedOrder.items.map((item, idx) => (
                          <option key={item._id} value={item._id}>
                            #{idx + 1}: {item.title} ({item.quantity} units - {item.paperType || 'Standard'})
                          </option>
                        ))}
                      </select>
                    </div>
              ...
              <div className="pt-2 flex justify-end gap-3 border-t border-slate-200">
                ...
                >
                  {handoffSubmitting && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
