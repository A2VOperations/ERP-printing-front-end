  ...
  const fetchNotes = async () => {
    try {
      ...
      if (res && res.data) {
        setNotes(
          res.data
            .filter((f) => f.notes || f.outcomeNotes)
            .map((f) => ({
              id: f._id,
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Client communication notes, special production instructions, and
                audit log
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredNotes.map((note) => (
              <div
                key={note.id}
                className="bg-white rounded-md p-5 border border-slate-200 shadow-xs space-y-3"
              >
          ...
          {filteredNotes.length === 0 && !loading && (
            <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-xs text-slate-400 text-xs">
              No notes found. Click &quot;Add Note&quot; to create a new client
              remark.
            </div>
          )}
        </div>
