    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="overflow-y-auto p-4 space-y-4">
          {query.trim().length >= 2 && results.customers.length === 0 && results.leads.length === 0 && !loading && (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-semibold text-blue-400">{c.customerNumber}</span>
              ...
              <div className="space-y-1">
                {results.leads.map((l) => (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-semibold text-emerald-400">{l.leadNumber}</span>
                        <span className="font-medium text-white">{l.contactName}</span>
                        {l.businessName && <span className="text-xs text-zinc-400">({l.businessName})</span>}
                      </div>
