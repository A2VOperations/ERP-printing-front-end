...
export default function LeaderboardView({ user }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [achRes, leadRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/v1/targets/my-achievement`, {
          headers: getHeaders(),
        }),
        fetch(`${API_BASE_URL}/api/v1/targets/leaderboard`, {
          headers: getHeaders(),
        }),
      ]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {achievement && (
        <div className="bg-gradient-to-br from-sky-600 to-indigo-700 p-6 rounded-md text-white shadow-lg space-y-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="text-3xl font-extrabold mt-1">
                ₹
                {((achievement.achievedPaise || 0) / 100).toLocaleString(
                  "en-IN",
                )}{" "}
                / ₹
                {((achievement.targetPaise || 0) / 100).toLocaleString("en-IN")}
              </div>
            </div>
          ...
          <div className="w-full bg-white/20 h-3 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, achievement.achievementPercent || 0)}%`,
              }}
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-2 border-t border-white/10 text-xs">
            ...
            <div>
              ...
              <div className="text-lg font-bold">
                ₹
                {((achievement.remainingPaise || 0) / 100).toLocaleString(
                  "en-IN",
                )}
              </div>
            </div>
            <div>
              ...
              <div className="text-lg font-bold">
                ₹
                {((achievement.requiredPerDayPaise || 0) / 100).toLocaleString(
                  "en-IN",
                )}
                /day
              </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        ...
        {leaderboard && !leaderboard.enabled ? (
          <div className="p-8 text-center text-slate-500 text-sm">
            {leaderboard.message ||
              "Leaderboard is disabled for this organization."}
          </div>
        ) : (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="px-6 py-8 text-center text-slate-400"
                    >
                      No sales data recorded for current cycle.
                    </td>
                  </tr>
                ) : (
                  leaderboard?.rankings?.map((r) => {
                    ...
                    return (
                      <tr
                        key={r.user?._id || r.rank}
                        className={`hover:bg-slate-50 ${isTop3 ? "bg-amber-50/20" : ""}`}
                      >
                        ...
                        <td className="px-6 py-4 font-bold text-slate-900">
                          ₹
                          {((r.achievedPaise || 0) / 100).toLocaleString(
                            "en-IN",
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-20 bg-slate-200 h-2 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, r.achievementPercent || 0)}%`,
                                }}
                              />
                            </div>
