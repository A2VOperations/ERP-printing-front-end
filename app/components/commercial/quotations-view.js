...
export default function QuotationsView({ user }) {
  ...
  const [items, setItems] = useState([
    { title: "Brochure Printing (A4, 300 GSM)", quantity: 1000, unitRate: 5.0, discountPercent: 0, taxRatePercent: 18 },
  ]);
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/quotations?${query.toString()}`, {
        headers: getHeaders(),
      });
  ...
  const handleAddItem = () => {
    setItems([...items, { title: "", quantity: 1, unitRate: 0, discountPercent: 0, taxRatePercent: 18 }]);
  };
  ...
  const previewGross = items.reduce((acc, it) => acc + (Number(it.quantity || 0) * Number(it.unitRate || 0)), 0);
  ...
  const handleCreateQuotation = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: `Quotation ${json.data.quotationNumber} created successfully!` });
  ...
  const handleSend = async (quoteId) => {
    try {
      setActionLoading(true);
      const res = await fetch(`${API_BASE_URL}/api/v1/quotations/${quoteId}/send`, {
        method: "POST",
        headers: getHeaders(),
      });
      const json = await res.json();
  ...
  const handleAccept = async (quoteId) => {
    if (!confirm("Are you sure you want to mark this quotation ACCEPTED and convert it into a confirmed Order?")) return;
    try {
      setActionLoading(true);
      const res = await fetch(`${API_BASE_URL}/api/v1/quotations/${quoteId}/accept`, {
        method: "POST",
        headers: getHeaders(),
      });
      const json = await res.json();
      if (json.success) {
        alert(`Quotation accepted! Order ${json.data?.order?.orderNumber} created successfully.`);
        fetchQuotations();
      } else {
  ...
  const handleDownloadPdf = async (quoteId, quoteNum) => {
    try {
      const token = localStorage.getItem("token") || "";
      const res = await fetch(`${API_BASE_URL}/api/v1/quotations/${quoteId}/pdf`, {
  ...
  const getStatusBadge = (status) => {
    const map = {
      ...
      PENDING_DISCOUNT_APPROVAL: "bg-amber-100 text-amber-800 border-amber-300 animate-pulse",
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Quotation Management</h1>
          <p className="text-sm text-slate-500 mt-1">
            Create, version, and manage commercial proposals with server-authoritative GST calculations.
          </p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-slate-400">Loading quotations...</td>
                </tr>
              ) : quotations.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-slate-400">No quotations found.</td>
                </tr>
              ) : (
                quotations.map((q) => (
                  <tr key={q._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{q.customerSnapshot?.displayName || "N/A"}</div>
                      <div className="text-xs text-slate-400">{q.customerSnapshot?.phone || "No phone"}</div>
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-600">
                      {q.items?.length || 0} line {q.items?.length === 1 ? "item" : "items"}
                    </td>
                    <td className="px-6 py-4 font-semibold text-slate-900">
                      ₹{((q.grandTotalPaise || 0) / 100).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button
                        onClick={() => handleDownloadPdf(q._id, q.quotationNumber)}
            ...
            <form onSubmit={handleCreateQuotation} className="space-y-4">
              ...
              <div className="border rounded-xl p-4 bg-slate-50 space-y-3">
                <div className="flex justify-between items-center">
                  <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Line Items Specification</h3>
                  <button
                    type="button"
                ...
                {items.map((it, idx) => (
                  <div key={idx} className="grid grid-cols-1 sm:grid-cols-12 gap-2 bg-white p-3 rounded-lg border border-slate-200 items-end">
                    <div className="sm:col-span-4">
                      <label className="block text-[11px] font-medium text-slate-600">Item Title / Spec</label>
                      <input
                        type="text"
                        required
                        value={it.title}
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[11px] font-medium text-slate-600">Rate (₹)</label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[11px] font-medium text-slate-600">Disc %</label>
                      <input
                        type="number"
                        min="0"
                        max="100"
                    ...
                    <div className="sm:col-span-1">
                      <label className="block text-[11px] font-medium text-slate-600">Tax %</label>
                      <input
                        type="number"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  {actionLoading ? "Calculating & Saving..." : "Create Quotation"}
                </button>
              </div>
            </form>
          </div>
