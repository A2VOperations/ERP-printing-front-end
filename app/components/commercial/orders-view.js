...
export default function OrdersView({ user }) {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/v1/orders?${query.toString()}`,
        {
          headers: getHeaders(),
        },
      );
  ...
  const getPaymentStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 text-[11px] rounded border ${map[status] || "bg-gray-100 text-gray-700"}`}
      >
        {status}
      </span>
    );
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Confirmed jobs with advance gate clearance, payment reconciliation,
            and tax invoices.
          </p>
        </div>
      </div>
                    ...
                    >
                      <td className="px-6 py-4 font-mono font-medium text-slate-900">
                        ...
                        <div className="text-xs text-slate-400 font-sans">
                          {o.invoiceNumber || "No Inv"}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-medium text-slate-900">
                          {o.customerSnapshot?.displayName || "N/A"}
                        </div>
                      ...
                      <td className="px-6 py-4 font-semibold text-slate-900">
                        ₹
                        {((o.grandTotalPaise || 0) / 100).toLocaleString(
                          "en-IN",
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-[11px] text-slate-500">
                              <span>
                                Req: ₹
                                {(
                                  (o.advanceRequiredPaise || 0) / 100
                                ).toLocaleString("en-IN")}
                              </span>
                              <span>
                                Rec: ₹
                                {(
                                  (o.advanceReceivedPaise || 0) / 100
                                ).toLocaleString("en-IN")}
                              </span>
                            </div>
                      ...
                      <td className="px-6 py-4 font-semibold text-rose-700">
                        ₹
                        {((o.balancePaise || 0) / 100).toLocaleString("en-IN", {
                          minimumFractionDigits: 2,
                        })}
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        <button
                          onClick={() =>
                            handleDownloadInvoice(o._id, o.orderNumber)
                          }
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-md max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 shadow-2xl">
            <div className="flex justify-between items-center border-b pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Invoice: {selectedOrder.invoiceNumber || "N/A"}
                </p>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs">
              <div className="border rounded-xl p-3 bg-slate-50">
                <h3 className="font-bold text-slate-700 uppercase mb-2">
                ...
                <div className="space-y-2">
                  {selectedOrder.items?.map((it, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between border-b pb-1 text-slate-700"
                    >
                      <div>
                        <div className="font-medium">{it.title}</div>
                        <div className="text-[11px] text-slate-400">
                          {it.quantity} units @ ₹
                          {((it.unitRatePaise || 0) / 100).toFixed(2)}
                        </div>
                      </div>
                      <div className="font-semibold">
                        ₹{((it.itemTotalPaise || 0) / 100).toFixed(2)}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-slate-100 p-3 rounded-xl space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">
                    ₹{((selectedOrder.grandTotalPaise || 0) / 100).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Total Paid:</span>
                  <span className="text-emerald-700 font-semibold">
                    ₹{((selectedOrder.totalPaidPaise || 0) / 100).toFixed(2)}
                  </span>
                </div>
