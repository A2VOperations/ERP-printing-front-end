...
export default function CustomerListView({ user }) {
  ...
  const [duplicateCheckInput, setDuplicateCheckInput] = useState({ phone: "", email: "", companyName: "" });
  ...
  const handleDuplicateCheck = async (e) => {
    ...
    try {
      const res = await apiClient.post("/api/v1/customers/check-duplicate", duplicateCheckInput);
    ...
    try {
      ...
      if (res.success) {
        ...
        setFormData({
          ...
          status: "PROSPECT",
          profileNotes: "",
          billingAddress: { addressLine1: "", city: "", state: "", pincode: "" },
        });
        fetchCustomers();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-900/60 p-6 rounded-2xl border border-zinc-800 backdrop-blur-xl shadow-xl">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">Manage permanent customer profiles, deduplication, and complete account histories.</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 relative">
          <svg className="w-5 h-5 absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl overflow-hidden backdrop-blur-xl shadow-xl">
        {loading ? (
          <div className="py-20 text-center">
            ...
            <p className="text-zinc-400 text-sm">Loading customer directory...</p>
          </div>
        ) : error ? (
          ...
          <div className="py-20 text-center text-zinc-400">
            ...
            <p className="text-base font-medium text-zinc-300 mb-1">No customers found</p>
            <p className="text-xs text-zinc-500">Create a new customer or convert inquiries to begin managing customer profiles.</p>
          </div>
        ) : (
                ...
                {customers.map((c) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      {c.phoneNormalized && (
                        <div className="text-xs text-zinc-500 font-mono">{c.phoneNormalized}</div>
                      )}
                    </td>
                    <td className="py-3.5 px-4 text-zinc-400">{c.email || "—"}</td>
                    <td className="py-3.5 px-4">
                      <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${getStatusBadge(c.status)}`}>
                        {c.status}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-600/10 hover:bg-blue-600 text-blue-400 hover:text-white border border-blue-500/20 hover:border-blue-600 rounded-lg text-xs font-medium transition inline-flex items-center gap-1.5"
                      >
        ...
        {pagination.totalPages > 1 && (
          <div className="p-4 border-t border-zinc-800 flex items-center justify-between text-xs text-zinc-400">
            <div>
              Total {pagination.totalRecords} customers (Page {pagination.page} of {pagination.totalPages})
            </div>
            ...
            <form onSubmit={handleDuplicateCheck} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setDuplicateCheckInput({ ...duplicateCheckInput, phone: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setDuplicateCheckInput({ ...duplicateCheckInput, email: e.target.value })}
                  className="w-full px-3.5 py-2 bg-zinc-800/80 border border-zinc-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setDuplicateCheckInput({ ...duplicateCheckInput, companyName: e.target.value })}
                  className="w-full px-3.5 py-2 bg-zinc-800/80 border border-zinc-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                />
              </div>
              ...
              >
                {isCheckingDuplicate ? "Checking Duplicate Database..." : "Run Duplicate Check"}
              </button>
            </form>
                    ...
                    {duplicateResult.exactPhoneMatch && (
                      <div className="p-2.5 bg-zinc-900 rounded-lg border border-zinc-700 text-xs text-zinc-300">
                        <div className="font-semibold text-white">
                          {duplicateResult.exactPhoneMatch.customerNumber}: {duplicateResult.exactPhoneMatch.displayName}
                        </div>
                        <div>Phone: {duplicateResult.exactPhoneMatch.phone} ({duplicateResult.exactPhoneMatch.phoneNormalized})</div>
                        <button
                          onClick={() => router.push(`/dashboard/customers/${duplicateResult.exactPhoneMatch._id}`)}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <svg className="w-5 h-5 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <form onSubmit={handleCreateCustomer} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setFormData({ ...formData, profileNotes: e.target.value })}
