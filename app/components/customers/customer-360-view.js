...
export default function Customer360View({ customerId, user }) {
  ...
  const handleCompleteFollowup = async (e) => {
    ...
    try {
      await apiClient.post(
        `/api/v1/followups/${selectedFollowup._id}/complete`,
        completeForm,
      );
  ...
  if (error || !data) {
    return (
      <div className="p-8 bg-zinc-900 border border-zinc-800 rounded-md text-center space-y-4">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 rounded-3xl p-6 backdrop-blur-xl shadow-2xl space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-zinc-800/80">
          <div className="p-4 bg-zinc-800/40 rounded-md border border-zinc-800">
          ...
          <div className="p-4 bg-zinc-800/40 rounded-md border border-zinc-800">
            ...
            <div className="text-2xl font-bold text-emerald-400 mt-1">
              {followupSummary.completedFollowups}
            </div>
          </div>
          <div className="p-4 bg-zinc-800/40 rounded-md border border-zinc-800">
            <div className="text-xs text-zinc-400 font-medium">
            ...
            <div className="text-2xl font-bold text-amber-400 mt-1">
              {followupSummary.pendingFollowups}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 border-b border-zinc-800 pb-1">
          {[
            ...
            {
              id: "leads",
              label: `Sales Opportunities (${recentLeads.length})`,
            },
            {
              id: "followups",
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-md p-5 space-y-4 backdrop-blur-xl">
              ...
              <div className="space-y-3 text-sm text-zinc-300">
                ...
                <div>
                  ...
                  <p className="font-medium text-zinc-200">
                    {customer.assignedSalesId?.name || "Unassigned"} (
                    {customer.assignedSalesId?.email || "—"})
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-md p-5 space-y-4 backdrop-blur-xl">
              ...
              <p className="text-sm text-zinc-300 whitespace-pre-wrap">
                {customer.profileNotes ||
                  "No specific customer profile notes recorded yet."}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-zinc-900/60 border border-zinc-800 rounded-md overflow-hidden backdrop-blur-xl shadow-xl">
            {recentLeads.length === 0 ? (
              <div className="p-12 text-center text-zinc-400">
                <p className="text-sm">
                  No sales leads associated with this customer yet.
                </p>
                <button
                  onClick={() => setShowNewLeadModal(true)}
        ...
        {activeTab === "followups" && (
          <div className="bg-zinc-900/60 border border-zinc-800 rounded-md overflow-hidden backdrop-blur-xl shadow-xl">
            {recentFollowups.length === 0 ? (
              <div className="p-12 text-center text-zinc-400">
                <p className="text-sm">
                  No follow-ups scheduled for this account.
                </p>
                <button
                  onClick={() => setShowNewFollowupModal(true)}
                  className="mt-3 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-xl transition"
                  ...
                  <tbody className="divide-y divide-zinc-800/60 text-zinc-300">
                    {recentFollowups.map((f) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-zinc-400">
                          {f.assignedToId?.name || "Unassigned"}
                        </td>
        ...
        {activeTab === "activity" && (
          <div className="bg-zinc-900/60 border border-zinc-800 rounded-md p-6 backdrop-blur-xl shadow-xl">
            {activityTimeline.length === 0 ? (
              <p className="text-center text-zinc-500 text-sm py-8">
                No historical activity events recorded.
              </p>
            ) : (
      ...
      {showNewLeadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-md w-full max-w-xl p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <h3 className="text-lg font-bold text-white">
                Create New Lead for {customer.displayName}
              </h3>
              <button
      ...
      {showCompleteFollowupModal && selectedFollowup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-md w-full max-w-lg p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <h3 className="text-lg font-bold text-white">
                Complete Follow-up: {selectedFollowup.title}
              </h3>
              <button
      ...
      {showNewFollowupModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-md w-full max-w-md p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <h3 className="text-lg font-bold text-white">
                Schedule Follow-up
              </h3>
